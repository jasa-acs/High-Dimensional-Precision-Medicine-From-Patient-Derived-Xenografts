...
for (i in 1:length(trts.by.cancer)) {
  ...
  for (j in 1:length(nums)) {
    ...
    for (k in 1:length(outs)) {
      ...
      R[2] = sprintf("pdx.owl.linear(cancer.type = '%s', outcome = '%s', gene.data.file = '%s', input_dir = '%s', output_dir = '%s', c1s = c(0, 1), c2s = %s, outstring = '_owllinear_rna.csv')",
                     tumor_type, outcome, gene.data.file, dir1, dir1, c2string)
      CMD = paste(R, collapse = "\n")
      pathtoRfile = sprintf("%s_%s_%s_submit_owllinear_rna.R", tumor_type, outcome, nums[j])
