...
for (i in 1:length(trts.by.cancer)) {
  ...
  for (j in 1:length(nums)) {
    gene.data.file = paste(tumor_type, ".", nums[j], ".biomarkers.rna.feat.csv", sep = "")
    for (k in 1:length(outs)) {
      ...
      R[2] = sprintf("pdx.owl.kernel.smooth(cancer.type = '%s', outcome = '%s', gene.data.file = '%s', numgenes = %s, input_dir = '%s', output_dir = '%s', c1s = c(0, 1), c2s = %s, outstring = '_owlkernelsmooth_rna.csv')",
                     tumor_type, outcome, gene.data.file, nums[j], dir1, dir1, c2string)
      CMD = paste(R, collapse = "\n")
      pathtoRfile = sprintf("%s_%s_%s_submit_owlkernelsmooth_rna.R", tumor_type, outcome, nums[j])
