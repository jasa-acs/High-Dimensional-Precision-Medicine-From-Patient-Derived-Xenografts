...
pdx.dl.owl.linear = function(cancer.type, outcome, gene.data.file, input_dir, output_dir, c1s = c(0), c2s = NA, k = 5, seed = 1, strip = T, outstring = "_owllineardl.csv") {
  ...
  if (cancer.type == "overall") {
    #biomarkers$brca = 0; biomarkers$cm = 0; biomarkers$crc = 0; biomarkers$nsclc = 0; biomarkers$pdac = 0
  ...
  for (c1 in c1s) {
    ...
    for (c2 in c2s) {  
      ...
      for (f in 1:length(folds)) {
        ...
        ##train.clin = train.clin[which(apply(train.clin, 1, FUN=function(x) sum(is.na(x))==0)), ]
        ##train.bio = train.bio[which(apply(train.clin, 1, FUN=function(x) sum(is.na(x))==0)), ]
        ...
        for (d in 1:dim(trt.vars)[2]) {
          ...
          #print(dim(temp.mat))
          #print(dim(temp.mat.nona))
          trainname = sprintf("dllinear_train_%s_%s_%i.csv", cancer.type, outcome, ngene)
          testname = sprintf("dllinear_test_%s_%s_%i.csv", cancer.type, outcome, ngene)
          ...
          # owl.temp.py needs to read in train.csv, fit OWL, read in test.csv, predict using OWL, and return a 1/-1 for each row in test.bio
          ...
          moves.mat = cbind(moves.mat, temp.res)  # need to make sure dimension of temp.res works here
          ...
          #print(class(traindatmoves))
          #print(traindatmoves)
          #print(dim(traindatmoves))
        ...
        for (t in 1:dim(test.bio)[1]) {
          ...
          # I think maybe we need these two lines instead of the two above
          temp = rep(NA, nrow(merge.steps) - length(moves))
