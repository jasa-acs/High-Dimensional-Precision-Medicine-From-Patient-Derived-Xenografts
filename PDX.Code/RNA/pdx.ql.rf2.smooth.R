...
pdx.ql.rf2.smooth = function(cancer.type, outcome, gene.data.file, numgenes, input_dir, output_dir, k = 5, seed = 1, strip = T) {
  ...
  load("pred_vals_rf_rna.rda")

  #load("pred_vals_rf_full.rda")
  ...
  if (cancer.type != "overall") {
    ...
    clinical$new.resp = pred_vals_rf_rna[[ind1]][[ind2]][[1]]
    clinical$new.surv = pred_vals_rf_rna[[ind1]][[ind2]][[2]]
  }

  if (cancer.type == "overall") {
    ind2 = which(numgenes == c(50, 100, 500, 1000))

    clinical$new.resp = pred_vals_rf_full[[ind2]][[1]]
    clinical$new.surv = pred_vals_rf_full[[ind2]][[2]]
  }
  ...
  if (cancer.type == "overall") cls = c(0)
  ...
  for (c1 in c1s) {
    ...
    if (cancer.type == "overall") c2s = c(1)
  ...
  rownames(res) = NULL
  setwd(output_dir)
  output.name = paste(cancer.type, "_", outcome, "_", stri_sub(gene.data.file, 1, -5), "_qlrf2smooth_rna.csv", sep = "")
