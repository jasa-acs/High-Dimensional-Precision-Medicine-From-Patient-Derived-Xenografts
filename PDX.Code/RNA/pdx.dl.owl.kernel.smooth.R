...
pdx.dl.owl.kernel.smooth = function(cancer.type, outcome, gene.data.file, numgenes, input_dir, output_dir, c1s = c(0), c2s = NA, k = 5, seed = 1, strip = T, outstring = "_owlkernelsmoothdl.csv") {
    ...
    for (c2 in c2s) {  
      ...
      for (f in 1:length(folds)) {
        ...
        for (d in 1:dim(trt.vars)[2]) {
          ...
          trainname = sprintf("dlsmoothkernel_train_%s_%s_%i.csv", cancer.type, outcome, ngene)
          testname = sprintf("dlsmoothkernel_test_%s_%s_%i.csv", cancer.type, outcome, ngene)
