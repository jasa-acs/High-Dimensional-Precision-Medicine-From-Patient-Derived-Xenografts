...
pdx.collate = function(tumor_type, nums, input_dir, output_dir) {
  ...
  for (i in 1:length(csv.list)) {
    ...
    # ys: order the dataset according to gene.name  
  ...
  for (i in 1:dim(new.df)[1]) {
    ...
    # ys: get p-value (and other values) for a gene from many p-values (and other values) according to some criteria
  ...
  for (k in 1:length(nums)) {
    ...
    # !!!!!ys: order according to cca.pval first, using the property of stable
    # ys: order according to p-value
    final = new.df[order(new.df$cca.pval), ]
    ...
    # ys: then select accroding to cca.cor
    ...
    # ys: then just select the smallest 50 p-values
    # ys: what if there are more than 50 genes with 0 p-values?????????(we need to use cca.pval)
    # ys: possible way: first order using cca.pval, then order using p.val
    if (num_genes >= num_zero) {
    ...
    filename = paste(tumor_type, ".", num_genes, ".results.csv", sep = "")
    ...
    filename2 = paste(tumor_type, ".", num_genes, ".biomarkers.csv", sep = "")
