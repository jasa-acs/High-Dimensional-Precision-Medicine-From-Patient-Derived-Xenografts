# Function takes a specified biomarker datafile and passed clinical variable dataset, then carries out the random 
#   forest analysis. Namely:
#   1) Process data and divide into 5 folds. For each outcome variable:
#       1a) For each fold: fit RF to the rest of the data, then use the fitted model to predict optimal treatments 
...
#       1b) Fit RF to entire data. Store variable importance.

rf.value <- function(infile,clin,verbose=F,rna_only=F,deeplearn=F,fulldata=F,seed=1) {
  library(randomForest)
  library(caret)
  dat <- read.csv(infile,header=T)
  keepcols <- grepl(".cn|.rna|.mut",colnames(dat))
  if (rna_only) {
    keepcols <- grepl(".rna",colnames(dat))
  }
  if (deeplearn) {
    keepcols <- grepl("feat",colnames(dat))
  }
  dat <- dat[,keepcols]
  ...
  X <- cbind(trt.mat,dat)
  if (fulldata) {X <- cbind(X,clin[,"TumorType"])}
  set.seed(seed)
  ...
  pred.cent.BAR <- rep(NA,nrow(X))
  pred.cent.TTD <- rep(NA,nrow(X))
  for (t in 1:2) {                                    # begin loop over outcome variables
    ...
    for (i in 1:length(folds)) {                     # begin loop over CV folds
      ...
      # Use random forest to predict outcome for each treatment in the ith fold and store predicted best treatments
    ...
    if (t==1) {
      ...
      obs_values_BAR <- Y[trt!="untreated"]
      obs_values_TTD <- Y2[trt!="untreated"]
    }
    if (t==2) {
      ...
      obs_values_BAR <- Y2[trt!="untreated"]
      obs_values_TTD <- Y[trt!="untreated"]
    }
    # Fit RF to full data and compute variable importance
