...
pdx.owl.linear.final = function(cancer.type, outcome, gene.data.file, input_dir, output_dir, c1, c2, strip = T, outstring = "_owllinear.coefs.csv") {
  ...
  for (d in 1:dim(trt.vars)[2]) {
    ...
    list.pred[[d]] = preds
        print(sprintf("coef_temp_owl_%s", trainname))
