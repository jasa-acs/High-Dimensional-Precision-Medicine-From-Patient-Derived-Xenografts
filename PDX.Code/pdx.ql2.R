...
pdx.ql2 = function(cancer.type, outcome, gene.data.file, input_dir, output_dir, k = 5, seed = 1, strip = T, outstring = "_ql2.csv") {
        ...
        for (d in 1:dim(trt.vars)[2]) {
          ...
          if (d > 1) {
            for (g in 1:dim(Y)[1]) {
              ...
              # ys: here is the difference between rf1 and rf2
