...
# OWL with linear decision function and random forest smoothed outcomes
...
pdx.owl.linear.smooth.final = function(cancer.type, outcome, gene.data.file, numgenes, input_dir, output_dir, c1, c2, k = 5, seed = 1, strip = T, outstring = "_owllinearsmooth.coefs.csv") {
  ...
  for (d in 1:dim(trt.vars)[2]) {
    ...
    trainname = sprintf("smoothlinear_train_%s_%s_%i.csv", cancer.type, outcome, ngene)
    testname = sprintf("smoothlinear_test_%s_%s_%i.csv", cancer.type, outcome, ngene)
    ...
    cmdstring = sprintf("python3 owl.final.linear.py %s %s", trainname, testname)
    ...
    list.pred[[d]] = preds
    temp.coefs = read.csv(sprintf("coef_temp_owl_%s", trainname), header = F)
    temp.coefs = as.matrix(temp.coefs)
    coefs = cbind(coefs, temp.coefs)
  ...
  #print(dim(coefs))
  #print(length(colnames(biomarkers)))
...
}  # end pdx.owl.linear.smooth.final function
