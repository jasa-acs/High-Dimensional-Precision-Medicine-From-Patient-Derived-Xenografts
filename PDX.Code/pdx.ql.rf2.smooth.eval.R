# pdx.ql.rf2.smooth.eval.R

# Q-learning with random forests and random forest smoothed outcomes, and pseudo-values
...
pdx.ql.rf2.smooth.eval = function(cancer.type, outcome, gene.data.file, numgenes, input_dir, output_dir, k = 5, seed = 1, strip = T, outstring = "_qlrf2smooth.csv") {
  ...
  for (f.sl in 1:length(folds.sl)) {
    ...
    for(i in 1:nrow(test.bio)){
      ...
      for(t in 1:ncol(test.clin)){
        ...
        if(isleft){
          predx = c(test.bio[i, ], 1)
        }else{
          predx = c(test.bio[i, ], 0)
        }
        
        # latent function as the prediction of the treatment group
        O[i,t] = predict(temp.fit, matrix(predx, nrow = 1))
      }
    }
