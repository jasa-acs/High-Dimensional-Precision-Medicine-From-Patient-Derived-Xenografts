...
pdx.ql.rf2.smooth.final = function(cancer.type, outcome, gene.data.file, numgenes, input_dir, output_dir, c1, c2, k = 5, seed = 1, strip = T, outstring = "_qlrf2smooth.coefs.csv") {
  ...
  if (outcome == "Surv") {
    ...
    smooth.clinical.other = smooth.resp.mat
        ...
        for (d in 1:dim(trt.vars)[2]) {
          ...
          fit = randomForest(tempx, tempy, importance = T)
          temp.imp = fit$importance[ , 2]
          list.models[[d]] = fit
        ...
        print(class(imps))
        print(dim(imps))
        write.csv(imps, output.name)
        
}  # end pdx.ql.rf2.smooth function
