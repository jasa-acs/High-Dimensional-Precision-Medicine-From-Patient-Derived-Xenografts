...
# Q-learning on PDX data with pseudo-values
...
pdx.ql2.smooth.final = function(cancer.type, outcome, gene.data.file, numgenes,input_dir, output_dir, c1, c2, k = 5, seed = 1, strip = T, outstring = "_ql2smooth.coefs.csv") {
  ...
  #original.resp = clinical$RespScaled
  #original.surv = clinical$logSurvScaled
  ...
  ##clinical$RespScaled = new.resp
  ##clinical$logSurvScaled = new.surv
  ...
  if (outcome == "Surv") {
    ...
    smooth.clinical.other = smooth.resp.mat
        ...
        train.bio = biomarkers
        train.clin = smooth.clinical
        test.bio = biomarkers
        test.clin = clinical
        test.clin.other = clinical.other
        ...
        for (d in 1:dim(trt.vars)[2]) {
          ...
          if (d > 1) {
            for (g in 1:dim(Y)[1]) {
              ...
              #if (!is.infinite(index)) Y[g, 1] = mean(as.numeric(list.pred[[index]][which(trt.vars[ , index] == list.pred[[index]][which(list.pred[[index]][ , 1] == rownames(Y)[g])))), 3], 2]), na.rm = T)
          ...
          list.pred[[d]] = preds
          coef.temp = coef(fit$glmnet.fit, s = lam)
          coef.temp = as.matrix(coef.temp)
          coefs = cbind(coefs, coef.temp)
        ...
        for (t in 1:dim(test.bio)[1]) {
          ...
          if (!to.treat) {
            ...
            trt.opt = c(trt.opt, "untreated")
          }
        }  # end loop through lines in test set
        
        coefs = coefs[-(1:(ncol(biomarkers)+2)), ]  # keep only the interaction w/ trt for each gene
        #coefs = matrix(coefs, ncol = 1)
        rownames(coefs) = colnames(biomarkers)
        
        print(coefs)
        
        setwd(output_dir)
        output.name = paste(cancer.type, "_", outcome, "_", stri_sub(gene.data.file, 1, -5), outstring, sep = "")
        write.csv(coefs, output.name)
        
        #return(coefs)
        
}  # end pdx.ql2.smooth function
