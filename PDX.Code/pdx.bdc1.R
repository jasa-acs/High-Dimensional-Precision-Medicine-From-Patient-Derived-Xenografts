...
pdx.bdc1 = function(tumor_type, trt_type, input_dir, output_dir) {
  ...
  # ys: here gene.names is not unique, there are names with multiple columns, and the values can be different(will cause problem?)
  ...
  # ys: is there a case when start_gene != 1?
  ...
  for (i in start_gene:end_gene) {
    ...
    X = NULL
    # ys: what's doing here? if there is model multiple times, it will be count more times than it should
  ...
  outfile = paste(output_dir, tumor_type, "_", trt_type, ".screen.csv", sep = "") 
